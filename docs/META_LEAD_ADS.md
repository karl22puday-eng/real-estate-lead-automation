...
## Option B — Meta webhook → n8n Webhook node (no native node) ✅ included

> **This option ships as a ready-to-import workflow:**
> [`workflows/03_meta_lead_intake.json`](../workflows/03_meta_lead_intake.json) (generated by
> [`scripts/build-meta-lead-workflow.ps1`](../scripts/build-meta-lead-workflow.ps1)). It
> already wires up everything below: the GET verification handshake (with verify-token
> check), an immediate 200 ack, Graph-API lead fetch, field mapping, and a forward into the
> main `realestate-intake` pipeline. **Setup:** set `$INTAKE` + `$VERIFY` at the top of the
> script and regenerate (or edit the nodes after import), attach a **Header Auth** credential
> to *Fetch Lead* (`Authorization: Bearer <PAGE_TOKEN>`), then register the webhook in Meta
> using the same `$VERIFY` token. The 10-node flow is exactly the steps below.

If you'd rather build it by hand (or understand what the workflow does):
